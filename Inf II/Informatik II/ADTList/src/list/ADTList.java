...
public abstract class ADTList<A> {
    ...
    private ADTSet<A> toSet(ADTList<A> list)
    {
	ADTList<A> a = this;
	ADTSet<A> n = ListSet.fromList(list);
	ListSet.fromList(list);
	//ADTSet<A> n = 
	return null;
    }
	...
	// return p.apply(list.head()) || list.tail().any(p, list);
...
//	    return list();
//	}
//	return p.apply(list.head()) ? new Cons<>(list.head(), list.tail()) : filter(p, list.tail());
