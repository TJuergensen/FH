...
//		// if head() is item searched for, kick twice, without adding to tempList. But still add it to temp List.
...
//		setElements = setElements.tail();
//	    }
//	    //setElemets is empty. Append tempList to reconstruct the list without but WITH the item.
...
//	ADTList<A> tempList = ADTList.list();
//	for (int i = 0; i < setElements.length(setElements); i++) {
//	    //Add all elements from elements to temp and delete elements. Double kick if e is wanted item..
//		if (setElements.head().equals(e))
//		    setElements = setElements.tail();
//		
//		tempList.cons(setElements.head());
...
//	return this;
    ...
    //Same as Subset, just another condition: of element of orig IS in s, return false
    ...
    {
	ADTList<A> orig = this.toList();
	//Just comparing these Lists on Equality would NOT do the job! Within sets, the position doesn't matter
	
	while(!orig.isEmpty())
	{
	    if(!s.member(orig.head()))
	    {
		return false;
	    }
	    orig = orig.tail();
